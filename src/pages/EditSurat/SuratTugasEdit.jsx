...
// Props: suratTugas (object data yang diedit), onClose, onSaved
function SuratTugasEdit({ suratTugas, onClose, onSaved }) {
  const [fileName, setFileName] = useState(
    suratTugas?.file_url ? suratTugas.file_url.split("/").pop() : ""
  );
  const [fileObj, setFileObj] = useState(null);
  const [saving, setSaving] = useState(false);
  const [formData, setFormData] = useState({
    nomorST: suratTugas?.nomor_st || "",
    tanggalSurat: suratTugas?.tanggal_surat || "",
    penerimaTugas: suratTugas?.penerima_tugas || "",
    tujuanTugas: suratTugas?.tujuan_tugas || "",
    tanggalMulai: suratTugas?.tanggal_mulai || "",
    tanggalSelesai: suratTugas?.tanggal_selesai || "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      let fileUrl = suratTugas?.file_url || null;

      if (fileObj) {
        if (suratTugas?.file_url) {
          const oldPath = suratTugas.file_url.split("/surat-tugas/")[1];
          if (oldPath) await supabase.storage.from("surat-tugas").remove([oldPath]);
        }
        const fileExt = fileObj.name.split(".").pop();
        const newFileName = `${Date.now()}_${formData.nomorST.replace(/\//g, "-")}.${fileExt}`;
        const { error: uploadError } = await supabase.storage.from("surat-tugas").upload(newFileName, fileObj);
        if (uploadError) throw uploadError;
        const { data: urlData } = supabase.storage.from("surat-tugas").getPublicUrl(newFileName);
      ...
      await Swal.fire({
        icon: "success",
        title: "Berhasil!",
        text: "Surat Tugas berhasil diperbarui.",
      ...
      <div style={s.modal}>
        ...
        <div style={s.cardHead}>
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            <div style={s.cardTitle}>Edit Surat Tugas</div>
