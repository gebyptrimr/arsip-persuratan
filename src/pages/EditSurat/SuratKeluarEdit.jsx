...
function SuratKeluarEdit({ suratKeluar, onClose, onSaved }) {
  const [fileName, setFileName] = useState(
    suratKeluar?.file_url ? suratKeluar.file_url.split("/").pop() : ""
  );
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      let fileUrl = suratKeluar?.file_url || null;

      if (fileObj) {
        if (suratKeluar?.file_url) {
          const oldPath = suratKeluar.file_url.split("/surat-keluar/")[1];
          if (oldPath) await supabase.storage.from("surat-keluar").remove([oldPath]);
        }
        const fileExt = fileObj.name.split(".").pop();
        const newFileName = `${Date.now()}_${formData.nomorSurat.replace(/\//g, "-")}.${fileExt}`;
        const { error: uploadError } = await supabase.storage.from("surat-keluar").upload(newFileName, fileObj);
        if (uploadError) throw uploadError;
        const { data: urlData } = supabase.storage.from("surat-keluar").getPublicUrl(newFileName);
      ...
      const { error } = await supabase.from("surat_keluar").update({
        ...
        file_url: fileUrl,
      }).eq("id", suratKeluar.id);
      if (error) throw error;

      await Swal.fire({ icon: "success", title: "Berhasil!", text: "Surat Keluar berhasil diperbarui.", confirmButtonColor: "#1A3A5C", confirmButtonText: "Oke" });
      onSaved?.();
      onClose?.();
    } catch (err) {
      ...
      <div style={s.modal}>
        ...
        <div style={s.cardHead}>
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            <div style={s.cardTitle}>Edit Surat Keluar</div>
            <div style={s.cardSub}>Surat Keluar · LP2M Universitas Negeri Makassar</div>
          </div>
          <div style={{ flex: 1 }} />
        ...
        <div style={s.cardBody}>
          <form onSubmit={handleSubmit}>
            <div style={s.sectionBlock}>
              <div style={s.sectionHead}><div style={s.sectionDot} /><span style={s.sectionLabel}>Identitas Surat Keluar</span></div>
              <div style={s.row}>
                <Field label="Nomor Surat" half required>
                  <input type="text" name="nomorSurat" value={formData.nomorSurat} onChange={handleChange} style={s.input} placeholder="Contoh: 001/SK/2025" required />
                </Field>
            ...
            <div style={s.sectionBlock}>
              ...
              <div style={s.row}>
                <Field label="Tujuan" half required>
                  <input type="text" name="tujuan" value={formData.tujuan} onChange={handleChange} style={s.input} placeholder="Contoh: BKD" required />
                </Field>
