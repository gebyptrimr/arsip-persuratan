...
function SipasButton({ type = "button", baseStyle, onClick, disabled, children }) {
  ...
  return (
    <button type={type} className="sipas-btn" onClick={onClick} disabled={disabled}
      onMouseDown={() => setPressed(true)} onMouseUp={() => setPressed(false)}
      onMouseLeave={() => setPressed(false)} onTouchStart={() => setPressed(true)} onTouchEnd={() => setPressed(false)}
      style={{ ...baseStyle, opacity: disabled ? 0.6 : pressed ? 0.85 : 1, transform: pressed ? "scale(0.985)" : "scale(1)", transition: "opacity 0.08s, transform 0.08s", backgroundColor: baseStyle.background, color: baseStyle.color, cursor: disabled ? "not-allowed" : "pointer" }}>
      {children}
    </button>
  );
}
...
function Field({ label, children, half, required }) {
  return (
    <div style={{ ...s.fieldWrap, width: half ? "calc(50% - 8px)" : "100%" }}>
      <label style={s.label}>{label}{required && <span style={s.requiredDot}>*</span>}</label>
      {children}
    </div>
  );
}

function SuratMasukEdit({ suratMasuk, onClose, onSaved }) {
  const [fileName, setFileName] = useState("");
  const [fileObj, setFileObj] = useState(null);
  const [saving, setSaving] = useState(false);
  const existingFileUrl = suratMasuk?.file_url || "";

  const [formData, setFormData] = useState({
    nomorSurat: suratMasuk?.nomor_surat || "",
    tanggalSurat: suratMasuk?.tanggal_surat || "",
    tanggalTerima: suratMasuk?.tanggal_terima || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (fileObj) {
        ...
        const newFileName = `${Date.now()}_${formData.nomorSurat.replace(/\//g, "-")}.${fileExt}`;
        const { error: uploadError } = await supabase.storage.from("surat-masuk").upload(newFileName, fileObj);
        if (uploadError) throw uploadError;
        const { data: urlData } = supabase.storage.from("surat-masuk").getPublicUrl(newFileName);
        fileUrl = urlData.publicUrl;

        if (existingFileUrl) {
          const oldPath = existingFileUrl.split("/surat-masuk/")[1];
          if (oldPath) await supabase.storage.from("surat-masuk").remove([oldPath]);
        }
      }
      ...
      await Swal.fire({ icon: "success", title: "Berhasil!", text: "Surat Masuk berhasil diperbarui.", confirmButtonColor: "#1A3A5C", confirmButtonText: "Oke" });
      onSaved?.();
      onClose?.();
    } catch (err) {
      ...
      <div style={s.modal}>
        ...
        <div style={s.cardHead}>
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            <div style={s.cardTitle}>Edit Surat Masuk</div>
            <div style={s.cardSub}>Surat Masuk · LP2M Universitas Negeri Makassar</div>
          </div>
          <div style={{ flex: 1 }} />
        ...
        <div style={s.cardBody}>
          <form onSubmit={handleSubmit}>
            ...
            <div style={s.sectionBlock}>
              <div style={s.sectionHead}><div style={s.sectionDot} /><span style={s.sectionLabel}>Identitas Surat Masuk</span></div>
              <div style={s.row}>
                <Field label="Nomor Surat" half required>
                  <input type="text" name="nomorSurat" value={formData.nomorSurat} onChange={handleChange} style={s.input} placeholder="Contoh: 001/SM/2025" required />
                </Field>
              ...
              <div style={s.row}>
                <Field label="Tanggal Diterima" half required>
                  <input type="date" name="tanggalTerima" value={formData.tanggalTerima} onChange={handleChange} style={s.input} required />
                </Field>
              </div>
            </div>
            ...
            {/* Informasi */}
            <div style={s.sectionBlock}>
              <div style={s.sectionHead}><div style={s.sectionDot} /><span style={s.sectionLabel}>Informasi Surat</span></div>
              <div style={s.row}>
                <Field label="Pengirim" half required>
                  <input type="text" name="pengirim" value={formData.pengirim} onChange={handleChange} style={s.input} placeholder="Nama Pengirim" required />
                </Field>
                <Field label="Perihal" half required>
                  <input type="text" name="perihal" value={formData.perihal} onChange={handleChange} style={s.input} placeholder="Perihal Surat" required />
                </Field>
              </div>
            </div>
            ...
            <div style={s.sectionBlock}>
              ...
              {existingFileUrl && !fileName && (
                <div style={s.currentFileBox}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#0C447C" strokeWidth="2">
                    <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/>
                  </svg>
                  <span style={{ fontSize: 12.5, color: "#0C447C", fontWeight: 600 }}>File saat ini tersimpan</span>
                  <a href={existingFileUrl} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "#4A9FD5", fontWeight: 600, marginLeft: "auto" }}>Lihat PDF</a>
                </div>
              )}
              <Field label={existingFileUrl ? "Ganti File PDF (opsional)" : "Upload File PDF"} required={!existingFileUrl}>
                <label style={s.uploadBox}>
                  <input type="file" accept=".pdf" required={!existingFileUrl} style={{ display: "none" }} onChange={handleFileChange} />
                  {fileName ? (
                    ...
                    <div style={s.uploadInner}>
                      <div style={s.uploadIconWrap}>
                        <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#4A9FD5" strokeWidth="1.6"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                      </div>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: "#1A3A5C" }}>{existingFileUrl ? "Klik untuk ganti file PDF" : "Klik untuk memilih file PDF"}</div>
...
const s = {
  ...
  label: { fontSize: 12, fontWeight: 600, marginBottom: 6, color: "#374151", letterSpacing: 0.1 },
  requiredDot: { color: "#E24B4A", marginLeft: 2 },
  input: { width: "100%", padding: "9px 12px", border: "0.5px solid #D1D5DB", borderRadius: 8, fontSize: 13, outline: "none", backgroundColor: "#FAFAFA", color: "#1A1A1A", boxSizing: "border-box" },
  currentFileBox: { display: "flex", alignItems: "center", gap: 8, backgroundColor: "#E6F1FB", border: "1px solid #B8D4EE", borderRadius: 8, padding: "9px 14px", marginBottom: 12 },
  ...
  actionBar: { display: "flex", justifyContent: "flex-end", gap: 10, borderTop: "1px solid #F0F0F0", paddingTop: 20, marginTop: 8 },
  btnBatal: { display: "inline-flex", alignItems: "center", background: "#F3F4F6", color: "#374151", border: "none", borderRadius: 8, padding: "9px 18px", fontSize: 13, fontWeight: 600, userSelect: "none" },
